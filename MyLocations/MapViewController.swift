...
//  MyLocations
//
//  Created by Nadya K on 09.12.2021.
//
...
class MapViewController: UIViewController {
            ...
            NotificationCenter.default.addObserver(forName: Notification.Name.NSManagedObjectContextObjectsDidChange,
              ...
              if self.isViewLoaded {
                if let dictionary = notification.userInfo {
                    self.updateLocations(for: dictionary)
                }
    ...
    func updateLocations(for dictionary: [AnyHashable: Any]){
        print(dictionary[NSInsertedObjectsKey])
        if let locs = dictionary[NSInsertedObjectsKey] as? Set<Location> {
            mapView.addAnnotations(Array(locs))
        }
        
        print(dictionary[NSUpdatedObjectsKey])
        if let locs = dictionary[NSUpdatedObjectsKey] as? Set<Location> {
            mapView.removeAnnotations(Array(locs))
            mapView.addAnnotations(Array(locs))
        }
        
        print(dictionary[NSDeletedObjectsKey])
        if let locs = dictionary[NSDeletedObjectsKey] as? Set<Location>  {
            mapView.removeAnnotations(Array(locs))
        }
    }
